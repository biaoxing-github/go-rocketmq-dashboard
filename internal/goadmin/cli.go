...
func runM6ShadowBatchCLI(ctx context.Context, options Options, args []string) int {
	fixturesJSON, fixturesFile, err := parseM6ShadowRunFlags(&options, args)
	if err != nil {
		_, _ = fmt.Fprintf(options.Stderr, "goadmin: %v\n", err)
		return 2
	}
	result, err := runM6ShadowBatch(ctx, options, fixturesJSON, fixturesFile)
	if err != nil {
		_, _ = fmt.Fprintf(options.Stderr, "goadmin: %v\n", err)
		return 1
	}
...
func parseM6ShadowRunFlags(options *Options, args []string) (string, string, error) {
	fixturesJSON := ""
	fixturesFile := ""
	seenRun := false
	for index := 0; index < len(args); index++ {
		...
		if arg == "--" {
			if index+1 < len(args) {
				return "", "", errors.New("--m6-shadow-run does not accept mqadmin command arguments")
			}
			break
		}
		if !strings.HasPrefix(arg, "--") {
			return "", "", fmt.Errorf("--m6-shadow-run does not accept mqadmin command argument %q", arg)
		}
		name, value, hasValue := strings.Cut(arg, "=")
		switch name {
		case "--m6-shadow-run":
			seenRun = true
		case "--m6-shadow-plan":
			return "", "", errors.New("--m6-shadow-run and --m6-shadow-plan cannot be used together")
		case "--m6-shadow-fixtures":
			if !hasValue {
				index++
				if index >= len(args) {
					return "", "", errors.New("--m6-shadow-fixtures requires a value")
				}
				value = args[index]
			}
			fixturesJSON = value
		case "--m6-shadow-fixtures-file":
			if !hasValue {
				index++
				if index >= len(args) {
					return "", "", errors.New("--m6-shadow-fixtures-file requires a value")
				}
				value = args[index]
			}
		...
		case "--namesrv":
			if !hasValue {
				index++
				if index >= len(args) {
					return "", "", errors.New("--namesrv requires a value")
				}
				value = args[index]
			}
			options.NameServer = strings.TrimSpace(value)
		case "--sidecar-addr":
			if !hasValue {
				index++
				if index >= len(args) {
					return "", "", errors.New("--sidecar-addr requires a value")
				}
				value = args[index]
			}
			options.SidecarAddr = strings.TrimSpace(value)
		case "--timeout-ms":
			...
			millis, err := strconv.Atoi(strings.TrimSpace(value))
			if err != nil || millis <= 0 {
				return "", "", fmt.Errorf("invalid --timeout-ms %q", value)
			}
			options.Timeout = time.Duration(millis) * time.Millisecond
			options.SidecarTimeout = options.Timeout
		default:
			return "", "", fmt.Errorf("unknown --m6-shadow-run flag %q", name)
		}
	}
	if !seenRun {
		return "", "", errors.New("--m6-shadow-run flag is required")
	}
	return fixturesJSON, fixturesFile, nil
}

func runM6ShadowBatch(ctx context.Context, options Options, fixturesJSON string, fixturesFile string) (nativeadmin.ShadowBatchResult, error) {
	samples := nativeadmin.DefaultM6ShadowPlan()
	resolvedFixturesJSON, err := readM6ShadowFixtures(fixturesJSON, fixturesFile)
	if err != nil {
	...
	batch := nativeadmin.ShadowBatch{
		...
		Targets: []nativeadmin.ShadowTarget{
			{Name: string(nativeadmin.ShadowProviderSidecar), Runner: m6ShadowTransportRunner{options: options, transport: "sidecar"}},
			{Name: string(nativeadmin.ShadowProviderNative), Runner: m6ShadowTransportRunner{options: options, transport: "native"}},
			{Name: string(nativeadmin.ShadowProviderAuto), Runner: m6ShadowTransportRunner{options: options, transport: "auto"}},
		},
		Samples:        samples,
...
Usage:
  ...
  goadmin --m6-shadow-run --m6-shadow-fixtures-file fixtures.json
